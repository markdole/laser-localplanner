...
class LocalPlanner():
    ...
    def move_core(self, goal_pi= Pose2D()):
        ...
        Kp_a = 0.1
        Ki_a = 0.1
        last_angular_x_err = 0.0
        while self.finished == False:
            #---------------#
            #self.sub_odom.callback()# TODO: deal with the callback fcn
            #self.link_angle()
            angle_error= goal_pi.theta- self.odom_pos_2d.theta
            angular_z_demand = cmp(angle_error, 0 )* (min( abs(angle_error)* 0.86 , 1.82))
            angular_z_err = angular_z_demand - self.odom.twist.twist.angular.z
            angular_z_increase = Kp_a * (angular_z_err - last_angular_x_err) + Ki_a * angular_z_err
            self.twist.angular.z = self.odom.twist.twist.angular.z + angular_z_increase
            last_angular_x_err = angular_z_err
    ...
    def point_distinguish(self, points):
        ...
        if len(self.front_obstacle[1] ) < 2:  # fixme :any danger ? good boy
            self.is_free_to_go = True
        else:
    ...
    def detect_block(self):
        ...
        if self.is_free_to_go:
            self.move_core(self.goal_pose_2d)
            return
        else:
