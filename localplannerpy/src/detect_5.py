...
class LocalPlanner():
    ...
    def __init__(self):
        ...
        self.path = [self.goal.pose]# done: clarify the class of path points
    ...
    def cbGoal(self, msg):
        ...
        self.list_H = [[goal_point,[0.0, 0.0]]]
        self.list_T = [[goal_point,[0.0, 0.0]]]
        self.path = [self.goal.pose]
        self.detect_block()
        self.letsmove()## fixme: move rate and turning behavior

    def move_core(self, pose= PoseStamped()):
        return pose
        self.sub_odom.callback()
        error = pose.pose.position.- self.odom_yaw
        #TODO: move core function with PID


    def letsmove(self):
        if not self.is_free_to_go:
            self.move_core(self.path[-1])
            self.path.pop()
            return
        else:
            self.move_core(self.path)
...
#                left_part.append(i)
        ...
        #------------#
        v_pm = self.pol2cart(left_part[0])
        v_pmg_max = v_g - v_pm
        ...
        for i in left_part[1:]:
            ...
            if theta_pi > theta_p_max:
                self.list_H.append([v_pi,[numpy.linalg.norm(v_pi), numpy.linalg.norm(v_ppm)]])
                break # the ansewer
        self.list_H.append([v_pm,[numpy.linalg.norm(v_pmg_max), left_part[0][0]]])
        #------------#
        ...
        for i in right_part[1:]:
            ...
            if theta_qi < theta_q_max:
                self.list_T.append([v_qi,[numpy.linalg.norm(v_qi), numpy.linalg.norm(v_qmi)]])
                break
        self.list_T.append([v_qm,[numpy.linalg.norm(v_qg_max), right_part[0][0]]])
        ...
        if direction == -1:# left
            for i in self.list_H:
                turnpoint.append([i[0].tolist(), left_part[0][1]])
        elif direction == 1:# right
            for i in self.list_T:
                turnpoint.append([i[0].tolist(), right_part[0][1]])
        else:
            turnpoint = []
        pose = PoseStamped()
        for i in turnpoint:
            pose.pose.position.x = i[0][0] + self.odom_pos.x
            pose.pose.position.y = i[0][1] + self.odom_pos.y
            pose.pose.position.z = 0.
            pose.pose.orientation.y = self.bean_to_angle(i[1]) + self.odom_yaw
            self.path.append(pose.pose)# done: send out the Path
